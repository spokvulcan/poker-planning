...
**Status:** superseded by [ADR-0026](0026-the-retro-is-a-whiteboard-like-the-poker-room.md): AgileKit sends no retro email. `convex/email.ts` sends the magic link only; the nudge, the reminders, the cards-due date, the opt-out, and the unsubscribe link and page are gone. `users.emailOptOut` remains in the schema, unused, and `UNSUBSCRIBE_SECRET` is no longer read.

Originally: accepted — decided on [map #253](https://github.com/spokvulcan/poker-planning/issues/253) via [#274](https://github.com/spokvulcan/poker-planning/issues/274). Built: the channel, opt-out and nudge in [#297](https://github.com/spokvulcan/poker-planning/issues/297), the reminders in [#298](https://github.com/spokvulcan/poker-planning/issues/298). Amended by [ADR-0024](0024-a-retro-shows-facts-and-never-a-measure-of-the-team.md): the in-app listings of retros in `collect` show the due date and the viewer's own "You haven't added a card yet" hint, never a card count, which lives on the board only. The privacy-policy references below are by file section title: "send sign-in emails" sits in *§4 How We Use Your Information* and the rights sentence in *§10 Your Rights and Choices* (confirmed while assembling `docs/spec/retro.md`, §23).
...
- **The mutation records intent and schedules; the action sends.** A nudge or reminder mutation writes its state (`lastNudge`, `reminderJobId`) and calls `ctx.scheduler.runAfter(0, internal.email.send, …)`. *As built (#298)*: the reminders schedule a sibling action, `internal.email.sendReminder`, because their job names an action item rather than a room and a sender; the due-date job uses `runAt`. Same channel, same send-time resolution. The action resolves recipients at send time through `ctx.runQuery`, skipping anyone opted out, deleted, address-less or no longer a team member, so a person who leaves between click and delivery is never emailed and no reference to a deleted account is ever followed. One Resend call per recipient. `convex/email.ts` grows into a small `send` with per-kind templates; no SDK, no send log table.
