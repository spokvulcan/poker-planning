...
- Account-level user events are the one exception to the bump rule: deleting or merging a user deletes/re-points `individualVotes` across rooms the user may no longer belong to, and an account event is not room liveness. Those paths invalidate the affected rooms' snapshot rows directly (`invalidateRoomAnalyticsSnapshots`), so the next read falls back to the scan.
