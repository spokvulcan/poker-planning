...
Online planning poker and retros for Scrum teams. This context covers the domain language teams and code share — rooms, roles, who is allowed to do what, the **voting round** a poker room runs, and the **steps** a **retro** moves through.
...
**Action**:
What an actor is attempting. Either a **category action** (one of the room type's configurable categories) or a **relationship action** (`remove` / `promote` / `demote`, which constrain the target's role; `transfer` / `changePerms` / `delete`, which do not).
...
**Permission level**:
The configurable threshold on a **permission category**: `everyone`, `facilitators`, or `owner`. Set by the owner. A poker room defaults every category to `everyone`, so new rooms behave as before; a retro defaults every category but `actionManagement` to `facilitators`.
_Avoid_: access level, role requirement

**Permission category**:
One of the four owner-configurable buckets of actions a room type has. A poker room's are **reveal cards**, **game flow**, **issue management**, **room settings**; a retro's are `stageFlow` (running the **steps**, the **discussion** and the **next retro**), `cardManagement` (someone else's revealed **stickies**), `actionManagement` (**action items**) and `retroSettings` (name, **columns**, **vote budget**, **show authors**) ([ADR-0026](docs/adr/0026-the-retro-is-a-whiteboard-like-the-poker-room.md)). The set is chosen by `roomType`; the **permission decision** is the same function over either. Each category carries one **permission level**.
...
Both are built and share one room route, one join flow and one canvas. See [ADR-0014](docs/adr/0014-retro-is-the-second-ceremony-of-one-toolkit.md) (in part) and [ADR-0026](docs/adr/0026-the-retro-is-a-whiteboard-like-the-poker-room.md).

**Ceremony**:
One of the Scrum meetings AgileKit hosts: **Planning poker** and **Retro**. A room has exactly one, named by its `roomType`. A documentation word only: it never appears in the product, where each ceremony is called by its own name.
...
**Retro**:
The reflection ceremony: a **room** of `roomType: "retro"` whose canvas is a shared whiteboard of **stickies** under **columns**, moved through four **steps**, and drawn with the poker room's canvas, chrome and join flow. "Retro" is the product word everywhere a user reads it; "retrospective" is reserved for long-form copy, titles and metadata.
_Avoid_: retrospective (in UI), retro room (a retro *is* a room), meeting, session, team retro (the retired alpha)

### Retro

Built as a whiteboard on the poker room's canvas. See [ADR-0026](docs/adr/0026-the-retro-is-a-whiteboard-like-the-poker-room.md). The team retro's words for its own objects (Team, stage, card, cluster, dot, raise, attribution) are retired for the retro; they survive only in legacy data and in the category names `stageFlow` and `cardManagement`.

**Step**:
Where a retro is: `write`, `vote`, `discuss` or `done`. Stored on the room and moved by a person who may run the retro, forward or back; nothing moves it by itself. A step decides what is **face-down**, when **votes** are cast, when their totals show and whether the **discussion** runs. It never locks a **sticky** against its author, nor an **action item**.
_Avoid_: stage (the team retro's word), phase (that is the **voting round**'s), mode, status

**Column**:
One prompt a retro asks, such as "Went well" or "To improve": a title, an emoji and a colour. Every **sticky** is filed under one column, which gives it its colour; where the sticky sits on the board is layout, not content. Columns are renamed, added (up to six) or removed while empty, on the board.
_Avoid_: prompt, category, lane, bucket

**Template**:
A named set of **columns** a retro starts from, such as "Start, Stop, Continue"; five ship. Copied into the retro when it is created and never referenced again.
_Avoid_: format (the team retro's editable format with stages), technique

**Pad**:
A **column**'s place on the board: its title on a pile of blank stickies. Clicking it starts a new **sticky** under that column, below the column's lowest one.
_Avoid_: column header, lane

**Sticky**:
One contribution to a retro: words, a GIF or both, filed under a **column** and placed anywhere on the board. Its author is always stored. The author edits or deletes it; so may someone who may manage other people's stickies, once it is revealed. Anyone in the retro may move it.
...
**Show authors**:
A retro's setting, off by default, that names each revealed **sticky**'s author on it. It hides names; it does not make a sticky anonymous. The author is always stored, and turning the setting on names stickies written while it was off.
...
**Topic**:
What a retro votes on and discusses: a loose **sticky**, or a whole **stack**.
...
**Vote budget**:
How many **votes** each person has in a retro: three by default, one to ten, set as "Votes per person" in the retro's settings. A vote beyond it is refused.
_Avoid_: dots, credits, points

**Discussion**:
The `discuss` step's walk through the **topics** that got votes, most votes first (every topic, column by column, when nobody voted), one at a time in the **spotlight**. The order is worked out from the current votes whenever it is read, never stored.
...
**Action item**:
Something the team agreed to do: a short text, at most one owner (someone in the retro), and done or not. It lives in its retro; the open ones are copied into the **next retro**. Never bare "action" — that word is the **permission decision**'s subject.
_Avoid_: action (see above), task, ticket, todo

**Next retro**:
The retro opened from a finished one by "Start the next retro": the same **columns**, **vote budget** and **show authors** setting, the number in the name moved on, the open **action items** **carried over**, and a link left on the finished retro. Opened at most once; whoever opens it owns it.
_Avoid_: series, recurring retro, follow-up

**Carried over**:
An **action item** copied into the **next retro** because it was still open. A copy, not a link: ticking it off does not tick off the original.
_Avoid_: imported, rolled over; carryover as a query (the team retro's)

**Summary**:
A retro as Markdown (action items, then the discussed topics in order, then every **sticky** by **column**), built in the viewer's browser from what that viewer can see, to copy or download. A retro has no other export.
...
**Guest retro**:
A retro that is not **retained**, in practice one a guest created, and so deleted like a poker room after five days without activity.
_Avoid_: temporary retro, anonymous retro (anonymous is an account type)
...
- **"Team"** is not a domain object any more: bare "team" means the people currently in a room ("the team voted") and is fine in copy. The capital-T Team of the retired team retro is gone; a `teamId` still on a room row is legacy data that the purge migration removes.
...
- **"Anonymous"** means an account type only: a guest's browser-session identity, as against a permanent account. A retro never makes a **sticky** anonymous: every sticky stores its author, and **show authors** only decides whether names are shown.
...
- **"Owner"**: the room's **owner** *Role* (exactly one per room) versus an **action item**'s owner (anyone in the retro, accountable for that item). Owning an action item confers no room power.
- **"Vote"**: a poker vote is the card a voter plays in a **voting round**; a retro **vote** is a mark on a **topic** within a **vote budget**. Different tables, different rules; say "retro vote" wherever the room type does not settle it.
- **"Reveal"**: in poker, the **transition** that settles a round and shows every card; in a retro, leaving `write`, which turns every **sticky** face-up. The same promise (nobody sees anything early), different mechanisms.
- **"Done"**: the retro's last **step** (`done`, shown as "Retro complete") versus an **action item** ticked done. Both can be undone.
- **`stageFlow` and `cardManagement`** are the retro's category names, kept from the team retro and its words: `stageFlow` governs the **steps**, the **discussion** and the **next retro**; `cardManagement` governs other people's **stickies** once they are revealed.
...
> **Dev:** Ann writes a **sticky** while the retro is in `write`. What does Bob's browser get?
>
> **Domain expert:** The sticky **face-down**: where it sits and its column's colour, nothing it says. The server decides that from the retro's **step**, so Bob's role doesn't matter; the owner gets the same.
>
> **Dev:** And once the facilitator moves the retro to `vote`?
>
> **Domain expert:** That's the **reveal**: every sticky is face-up for everyone. Ann's name still doesn't show unless the retro has **show authors** on, and if someone turns it on later it names her sticky too, because the author is always stored. So copy says names are hidden by default, never that stickies are anonymous.
>
> **Dev:** Bob drops his sticky on Ann's. How many topics is that?
>
> **Domain expert:** One. A **stack** is a single **topic**: a vote for either sticky counts for the stack, and the **discussion** reaches it once.
